...
class MCSEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        self.episode_unique_min_traj_dir = os.path.join(save_dir, 'episode_unique_min')
        ...
        if not os.path.exists(self.episode_unique_min_traj_dir):
            os.makedirs(self.episode_unique_min_traj_dir)
    ...
    def step(self, action):
		...
		#checking the similarilty of the relaxed cluster minimum  between cluster minima already found
            ...
            if any(bool_list): #if the minimum is already found, reward is zero
                ...
                reward +=   100 /  np.exp(self.relative_energy)
                self.done = True
    ...
    def save_traj(self):      
        save_path = os.path.join(self.traj_dir, '%d_%f_%f_%f_full.traj' %(self.episodes, self.minima['energies'][self.min_idx]
                                                                      , self.initial_energy, self.episode_reward))
        episode_min_path = os.path.join(self.episode_min_traj_dir, '%d_%d_%f_%f_%f_full.traj' %(self.episodes, self.minima['timesteps'][-1],self.minima['energies'][self.min_idx]
                                                                      , self.initial_energy, self.episode_reward))
        episode_unique_min_path = os.path.join(self.episode_unique_min_traj_dir, '%d_%d_%f_%f_%f_full.traj' %(self.episodes, len(self.unique_minima)
        ...
        write(episode_unique_min_path, self.unique_minima)
        #if reward > 1000:
            #large_reward_path = os.path.join(self.episode_min_traj_dir, 'zzz_%d_%f_large_reward.traj' %(self.episodes, self.episoe_reward))
        
        #write(large_reward_path, self.minima['minima'])
        return
