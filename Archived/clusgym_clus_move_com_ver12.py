...
import copy
from utils import checkSimilar, addAtoms, fixOverlap, checkBonded, checkOverlap
import itertools
...
class MCSEnv(gym.Env):
    ...
    def step(self, action):
        ...
        all_move_minimize = True

        #dist_after_move = self.atoms.get_all_distances()[self.atom_selection]
        #z1 = [ dist for k, dist in enumerate(dist_after_move) if k != self.atom_selection and dist < self.avg_radii*0.5 ]
	
        #if len(z1) > 0:     	#checking for overlapping atoms after movement
        #if checkOverlap(self.atoms) == True:
            #reward -= 100.0
        #elif checkBonded(self.atoms) == False:
            #reward -= 100.0
        if all_move_minimize: 			# all new cluster configuration minimization
            ...
            if checkBonded(self.atoms) == False:
                ...
                self.n_tot_all_minima = len(self.all_minima['minima']) -1
                #reward += 2**(self.n_tot_all_minima)
                reward += 100 * (self.n_tot_all_minima)
                ...
                if any(bool_list): #if the minimum is already found (similarity is True), reward is zero
                    reward += 0.0
                else:				# a new minima found
                    if self.relative_energy < 0.0: #Lower in energy than the initial clus energy
                        reward += (100 * np.exp((-1) * self.relative_energy)) * (self.n_lower_energy_minima)**2
                        self.n_lower_energy_minima += 1
                        #reward *= (self.n_lower_energy_minima)**2
                    else:
                    ...
                    self.n_new_min = len(self.minima['minima']) -1
                    reward += 10**(self.n_new_min)
