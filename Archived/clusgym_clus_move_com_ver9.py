...
class MCSEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if episode_over: 
            ...
            self.episodes += 1
            with open('episode_data.txt', "a+") as fh:
                fh.write(f"episode_number: {self.episodes}, "
                    f"episode_reward: {self.episode_reward}, "
                    f"total minima: {self.n_tot_all_minima}, "
                    f"total new minima: {len(self.minima['minima'])}, "
                    f"total lower energy minima: {self.n_lower_energy_minima} \n ")
